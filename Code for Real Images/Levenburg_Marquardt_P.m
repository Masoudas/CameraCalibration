function [Err_s,Err_f,extr_cur,wand_dis,x_cur,time]=Levenburg_Marquardt_P(weight,im_coordinate,extr_cur,focal_len,P_point,x_cur,M,N,marker_dis,itr_max,th)
%% This code optimizes the dynamic calibration function using penalty functions and
% Levenburg-Marquardt algorithm. The object is a wand
...
%             J(4,4)=weight(m,n)*(0);
        ...
        b(6*N+6*(m-1)+1:6*N+6*m,1)=b(6*N+6*(m-1)+1:6*N+6*m,1)+beta*I*(-((x_cur(2*m-1,1) - x_cur(2*m,1))^2 + (x_cur(2*m-1,2) - x_cur(2*m,2))^2 + (x_cur(2*m-1,3) - x_cur(2*m,3))^2 - marker_dis^2));
        H(6*N+1+6*(m-1):6*N+6*m,6*N+6*(m-1)+1:6*N+6*m)=H(6*N+1+6*(m-1):6*N+6*m,6*N+6*(m-1)+1:6*N+6*m)+beta*I*I';
    end
    H=H+mu*eye(6*M+6*N);
    delta_X=H\b;
