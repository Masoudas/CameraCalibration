...
MSE=1;             % Desired mean square error for the cost function
mu=0;               % Newton damping parameter, in the relation (H+mu*I)
itr_max=100;        % Maximum allowable iterations for dynamic calibration optimization
...
% Constrainted methods
[Err_s_SC(ceil(noise+0.5),test_n),Err_f_SC(ceil(noise+0.5),test_n),extr_cur_SC,wand_dis_SC,x_cur_SC(:,:,test_n),itr_SC(ceil(noise+0.5),test_n)]=Opt_SC(weight,x_cur,im_coordinate,extr_cur,P_point,focal_len,M,N_fp,N,marker_dis,itr_max,MSE);
[Err_s_LC(ceil(noise+0.5),test_n),Err_f_LC(ceil(noise+0.5),test_n),extr_cur_LC,wand_dis_LC,x_cur_LC(:,:,test_n),itr_LC(ceil(noise+0.5),test_n)]=Levenburg_Marquardt_L(weight,im_coordinate,extr_cur,focal_len,P_point,x_cur,M,N,marker_dis,itr_max,0.1);                                                                                                                
[Err_s_P(ceil(noise+0.5),test_n),Err_f_P(ceil(noise+0.5),test_n),extr_cur_P,wand_dis_P,x_cur_P(:,:,test_n),itr_P(ceil(noise+0.5),test_n)]=Levenburg_Marquardt_P(weight,im_coordinate,extr_cur,focal_len,P_point,x_cur,M,N,marker_dis,itr_max,MSE);

% Unconstrainted methods
[Err_s_L(ceil(noise+0.5),test_n),Err_f_L(ceil(noise+0.5),test_n),extr_cur_L,wand_dis_L,x_cur_L(:,:,test_n),itr_L(ceil(noise+0.5),test_n)]=Levenburg_Marquardt(weight,im_coordinate,extr_cur,focal_len,P_point,x_cur,M,N,marker_dis,itr_max,MSE);                                                                                                                
[Err_s_S(ceil(noise+0.5),test_n),Err_f_S(ceil(noise+0.5),test_n),extr_cur_S,wand_dis_S,x_cur_S(:,:,test_n),itr_S(ceil(noise+0.5),test_n)]=Opt_S(weight,x_cur,im_coordinate,extr_cur,P_point,focal_len,M,N_fp,N,marker_dis,itr_max,MSE);
...
% Seperation method
extr_cur_S(:,4:6)=extr_cur_S(:,4:6)/mean(wand_dis_S)*300;
...
% Constrainted separation method
x_sc=Trg(weight_t,im_test,extr_cur_SC,P_point,focal_len,M_t,N,N_fp); % Initial 3D coordinate estimation using triangulation as explained in the thesis.
                                                                                            % Details are given in the matlab file.
wand_sc(ceil(noise+0.5),test_n)=std(sqrt(sum(((x_sc(1:2:2*M_t,:)-x_sc(2:2:2*M_t,:)).^2)')')); % Marker distance for each set of 3D points.
wand_asc(ceil(noise+0.5),test_n)=mean(sqrt(sum(((x_sc(1:2:2*M_t,:)-x_sc(2:2:2*M_t,:)).^2)')')); % Marker distance for each set of 3D points.


% Lagrangian method
extr_cur_L(:,4:6)=extr_cur_L(:,4:6)/mean(wand_dis_L)*300;
...
% Constrainted Lagrangian method
x_lc=Trg(weight_t,im_test,extr_cur_LC,P_point,focal_len,M_t,N,N_fp); % Initial 3D coordinate estimation using triangulation as explained in the thesis.
                                                                                            % Details are given in the matlab file.
wand_lc(ceil(noise+0.5),test_n)=std(sqrt(sum(((x_lc(1:2:2*M_t,:)-x_lc(2:2:2*M_t,:)).^2)')')); % Marker distance for each set of 3D points.
wand_alc(ceil(noise+0.5),test_n)=mean(sqrt(sum(((x_lc(1:2:2*M_t,:)-x_lc(2:2:2*M_t,:)).^2)')')); % Marker distance for each set of 3D points.
