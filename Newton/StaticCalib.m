...
dis1(:,i+8)=sum([(AB-550)^2 (BC-200)^2 (BD-750)^ (CD-550)^2 (AD-sqrt(550^2+750^2))^2]); % Sum of the difference between estimated and actual length
...
tet(i+8,:)=[-theta_x -theta_y -theta_z]/pi*180;
Translation1(:,i+8)=(Rot_x*Rot_y*Rot_z*(T));
...
iprts=[im';ones(1,4)];
R=Rotate3(angle(1)/180*pi,angle(2)/180*pi,angle(3)/180*pi);
opt=R;
