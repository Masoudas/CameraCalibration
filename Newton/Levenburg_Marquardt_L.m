function [Err_s,Err_f,extr_cur,wand_dis,x_cur,time]=Levenburg_Marquardt_L(weight,im_coordinate,extr_cur,focal_len,P_point,x_cur,M,N,marker_dis,itr_max,th)
...
while (max(abs(delta_X(6*N+1:6*N+6*M)))>th && itr<itr_max)
    ...
    for m=1:M
        ...
        b(6*N+6*M+m)=-((x_cur(2*m-1,1) - x_cur(2*m,1))^2 + (x_cur(2*m-1,2) - x_cur(2*m,2))^2 + (x_cur(2*m-1,3) - x_cur(2*m,3))^2 - marker_dis^2);
        ...
        H(6*N+1+6*(m-1):6*N+6*m,6*N+6*M+m)=I;
        H(6*N+6*M+m,6*N+1+6*(m-1):6*N+6*m)=I';
    end
